    ...
    def post(self):
        ...
        if text.startswith('/'):
            if text.startswith('/start'):
                if getEnabled(chat_id) == False:
                    reply(msg=send_welcome())
                    setEnabled(chat_id, True)
                else:
                    reply(msg="Ya se encuentra activo.")
            elif text.startswith('/stop'):
                reply(msg=send_goodbye())
                setEnabled(chat_id, False)
            elif text.startswith('/decrypt'):
                if getEnabled(chat_id):
                    reply(msg=send_decrypt(message))
            elif text.startswith('/encrypt'):
                if getEnabled(chat_id) == True:
                    reply(msg=send_encrypt(message))
